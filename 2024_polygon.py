...
coordinates_df = pd.read_csv('2024coordinates.csv')
...
def is_valid_coordinate(lat, lng):
    ...
    # If either value is a string, check if it's empty or contains invalid patterns
    if isinstance(lat, str) and isinstance(lng, str):
        if lat.strip() == "" or lng.strip() == "":
            return False
        try:
            lat_float = float(lat)
            lng_float = float(lng)
            if lat_float == 0.0 or lng_float == 0.0:
                return False
        except ValueError:
            return False
...
with open('2024_polygon.geojson', 'w') as geojson_file:
